...
function App() {
  ...
  // Fetch shop for owner
  useEffect(() => {
    const fetchCurrentShop = async () => {
      if(userData?.role === "owner") {
        try {
          const res = await axios.get(`${serverUrl}/api/shop/getcurrent`, { withCredentials: true });
          if(res.data) dispatch(setShop(res.data))
        } catch(err) {
          console.log("fetchCurrentShop error:", err.response?.data || err.message)
        }
      }
    }
    fetchCurrentShop();
