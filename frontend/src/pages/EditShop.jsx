...
export default function EditShop() {
    ...
    useEffect(() => {
        if(shop) {
            setName(shop.name)
            setCity(shop.city)
            setStateName(shop.state)
            setAddress(shop.address)
            setFrontendImage(shop.image)
        }
    }, [shop])
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.post(`${serverUrl}/api/shop/editshop`, formData, { withCredentials: true })
            dispatch(setShop(res.data))
            alert("Shop updated successfully!")
            navigate("/")
        } catch(err) {
            console.log("Edit shop error:", err.response?.data || err.message)
        }
    }

    return (
        <div className="flex justify-center flex-col items-center p-6 bg-orange-50 min-h-screen">
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md space-y-4">
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Shop Name" required />
                <input type="text" value={city} onChange={e => setCity(e.target.value)} placeholder="City" required />
                <input type="text" value={stateName} onChange={e => setStateName(e.target.value)} placeholder="State" required />
                <textarea value={address} onChange={e => setAddress(e.target.value)} placeholder="Address" required />
                <input type="file" accept="image/*" onChange={handleImageChange} />
                {frontendImage && <img src={frontendImage} alt="Shop" className="w-full h-48 object-cover" />}
                <button type="submit" className="bg-orange-500 text-white py-2 rounded">Save</button>
            </form>
        </div>
    )
}
